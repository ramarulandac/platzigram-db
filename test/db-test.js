...
//  Ava uses hooks-> methos to execute before and after executing tests
...
test('save image', async t => {
  let db = t.context.db
  t.is(typeof db.saveImage, 'function', 'save image is function')
...
test('get image', async t => {
  ...
  //  throws-> if getimage errors.. looks for not found string in the error message
...
test('get user', async t => {
  ...
  t.deepEqual(created, result)
//  throws-> if getUser errors.. looks for not found string in the error message
...
test('list images by user', async t => {
  ...
  for (let i = 0; i < images.length; i++) {
    ...
    //  Just holding promises in an array -> those promises need to be resolved

   //  all promises are resolved below
  }
